...
<main
    ...
    <!-- ══════════════════════════════════════════
     MOBILE MENU — full-screen, right slide,
     two-overlay cinematic animation
══════════════════════════════════════════ -->
    <div id="mob-wrap" class="fixed inset-0 z-[90] pointer-events-none" aria-hidden="true">
        ...
        <!-- Overlay 1 — violet wipe (comes first, exits left) -->
        <div id="mob-ov1" class="absolute inset-y-0 right-0 w-full md:w-[420px] bg-violet z-30 translate-x-full"
            style="transition: transform 0.38s cubic-bezier(0.7, 0, 0.3, 1)"></div>

        <!-- Overlay 2 — dark panel (comes second, stays as menu bg) -->
        <div id="mob-panel"
            class="absolute inset-y-0 right-0 w-full md:w-[420px] bg-[#07080e] z-20 translate-x-full flex flex-col"
            style="transition: transform 0.42s cubic-bezier(0.7, 0, 0.3, 1)">
            <!-- Top bar -->
            <div class="flex items-center justify-between px-8 pt-8 pb-6 border-b border-white/[0.07]">
                <a href="#" onclick="closeMob()"
                    class="flex items-center gap-2.5 font-display text-[17px] font-semibold">
                    <span class="grid h-8 w-8 place-items-center rounded-lg bg-grad-main shadow-glow"><span
                            class="font-mono text-sm font-bold text-white">W</span></span>
                    <span class="text-white">Wilson<span class="text-lilac">.</span>dev</span>
                </a>
                <button onclick="closeMob()" aria-label="Close menu"
            ...
            <nav class="flex-1 flex flex-col justify-center px-8 gap-1" id="mob-nav">
              ...
              transition:
                opacity 0.4s,
                transform 0.4s;
            "><span class="font-mono text-xs text-violet">01</span><span
              ...
              transition:
                opacity 0.4s,
                transform 0.4s;
            "><span class="font-mono text-xs text-violet">02</span><span
              ...
              transition:
                opacity 0.4s,
                transform 0.4s;
            "><span class="font-mono text-xs text-violet">03</span><span
              ...
              transition:
                opacity 0.4s,
                transform 0.4s;
            "><span class="font-mono text-xs text-violet">04</span><span
                ...
                <a href="#contact" onclick="closeMob()"
                    class="mob-link group flex items-center gap-4 py-4 opacity-0 translate-x-8" style="
              transition:
                opacity 0.4s,
                transform 0.4s;
            "><span class="font-mono text-xs text-violet">05</span><span
            ...
            <!-- Bottom: socials + CTA -->
            <div id="mob-footer" class="px-8 pb-8 pt-6 border-t border-white/[0.07] opacity-0 translate-y-4" style="
            transition:
              opacity 0.4s,
              transform 0.4s;
          ">
...
══════════════════════════════════════════ -->
    <header id="nav" class="fixed inset-x-0 top-0 z-50 transition-all duration-300">
        <nav class="max-w-[1280px] mx-auto px-6 lg:px-10 h-16 flex items-center justify-between gap-6">
            <a href="#" class="group flex items-center gap-2.5 font-display font-semibold text-[17px] flex-shrink-0">
                <span
                    class="grid h-8 w-8 place-items-center rounded-lg bg-grad-main shadow-glow transition-transform duration-300 group-hover:-rotate-6 group-hover:scale-105"><span
                        class="font-mono text-[13px] font-bold text-white">W</span></span>
                <span class="text-[#0b1120] dark:text-white">Wilson<span class="text-lilac">.</span>dev</span>
            </a>
            ...
            <div class="flex items-center gap-2.5">
                <button id="tbtn" onclick="cycleTheme()" aria-label="Toggle theme"
                ...
                <a href="#contact" data-magnetic
                    class="hidden md:inline-flex items-center gap-2 rounded-full bg-grad-main px-5 py-2 text-[13px] font-medium text-white shadow-glow hover:shadow-glow-v hover:-translate-y-0.5 transition-all duration-200">Hire
                    Me →</a>
                <!-- Hamburger — animated to X -->
                <button id="hambtn" onclick="openMob()" aria-label="Open menu"
        ...
        <div class="max-w-[1280px] mx-auto px-6 lg:px-10 grid gap-10 py-14 md:grid-cols-3">
            ...
            <div class="text-sm text-[#8099b8] dark:text-white/50">
                ...
                <ul class="grid grid-cols-2 gap-2">
                    ...
                    <li><a href="#process"
                            class="hover:text-[#0b1120] dark:hover:text-white transition-colors">Process</a></li>
                    <li><a href="#contact"
                            class="hover:text-[#0b1120] dark:hover:text-white transition-colors">Contact</a></li>
                </ul>
            </div>
            <div>
...
══════════════════════════════════════════ -->
    ...
    /* ════ THEME ════════════════════════════════ */
    const TP = ["system", "light", "dark"];
    let ti = TP.indexOf(document.documentElement.getAttribute("data-p") || "system");

    function updateTUI(p) {
        ["i-sys", "i-sun", "i-moon"].forEach((id) => {
            const e = document.getElementById(id);
            if (e) {
                e.style.opacity = "0";
                e.style.transform = "scale(.4) rotate(40deg)";
            }
        });
        const map = {
            system: "i-sys",
        ...
        const a = document.getElementById(map[p]);
        if (a) {
            a.style.opacity = "1";
    ...
    function applyTheme(p) {
        const a = p === "system" ? (window.matchMedia("(prefers-color-scheme:dark)").matches ? "dark" : "light") : p;
        document.documentElement.classList.toggle("dark", a === "dark");
        document.documentElement.setAttribute("data-p", p);
        localStorage.setItem("wm-t", p);
        updateTUI(p);
        updateNav();
    }
    window.cycleTheme = () => {
    ...
    window.matchMedia("(prefers-color-scheme:dark)").addEventListener("change", () => {
        if (TP[ti] === "system") applyTheme("system");
    });
    updateTUI(TP[ti]);
    ...
    function updateNav() {
        ...
        if (!navEl) return;
        if (stuck) {
            navEl.style.borderColor = dk ? "rgba(255,255,255,.07)" : "rgba(0,0,0,.06)";
            navEl.style.background = dk ? "rgba(5,6,10,.82)" : "rgba(244,246,255,.9)";
    ...
    /* ════ MOBILE MENU (Two-Overlay Wipe) ═══════

   Animation sequence on OPEN:
   t=0ms   → backdrop dims
   t=20ms  → Overlay1 (violet) slides in from right
   t=250ms → Overlay2 (dark panel) slides in from right
             while Overlay1 slides out to LEFT
   t=550ms → Menu items stagger in from right

   Animation sequence on CLOSE:
   t=0ms   → items fade out
   t=100ms → Overlay2 (panel) slides out to right
   t=200ms → Overlay1 (violet) slides in from left, then out right
   t=480ms → backdrop fades, pointer-events removed
...
═══════════════════════════════════════════ */
    ...
    window.openMob = function() {
        ...
        // Step 2: Violet overlay flies in from right
        setTimeout(() => {
            mobOv1.style.transform = "translateX(0)";
        }, 20);

        // Step 3: Dark panel flies in, violet exits left
        setTimeout(() => {
            mobPanel.style.transform = "translateX(0)";
            mobOv1.style.transform = "translateX(-110%)";
        }, 260);

        // Step 4: Stagger menu items in
        setTimeout(() => {
    ...
    window.closeMob = function() {
        ...
        // Violet flashes in from left then exits right (outro wipe)
        setTimeout(() => {
        ...
        setTimeout(() => {
            mobOv1.style.transition = "transform .38s cubic-bezier(.7,0,.3,1)"; // reset
        }, 620);
    ...
    if (cur && !REDUCED) {
        ...
        document.querySelectorAll("a,button,[data-tilt],.bcard").forEach((el) => {
            el.addEventListener("mouseenter", () => {
                cur.style.width = "44px";
                cur.style.height = "44px";
                cur.style.borderColor = "rgba(181,158,245,.8)";
            });
            el.addEventListener("mouseleave", () => {
                cur.style.width = "32px";
                cur.style.height = "32px";
                cur.style.borderColor = "rgba(31,140,249,.7)";
            });
        });
    }

    /* ════ MOUSE PARALLAX — ambient orbs ════════ */
    ...
    if (!REDUCED)
        (function animG() {
            ...
            if (ambA) ambA.style.transform = `translateX(calc(-50% + ${gax}px)) translateY(${gay}px)`;
            if (ambB) ambB.style.transform = `translateY(${gby}px) translateX(${gbx}px)`;
            requestAnimationFrame(animG);
        })();

    /* ════ HERO WORD PARALLAX ═══════════════════ */
    const hw = document.getElementById("heroWord");
    window.addEventListener(
        "scroll",
        () => {
            if (hw) hw.style.transform = `translate(-50%,calc(-55% + ${window.scrollY * 0.13}px))`;
        }, {
            passive: true
        },
    );
    ...
    document.querySelectorAll("[data-magnetic]").forEach((el) => {
        el.addEventListener("mousemove", (e) => {
            ...
            el.style.transform =
                `translate(${(e.clientX - r.left - r.width / 2) * 0.25}px,${(e.clientY - r.top - r.height / 2) * 0.3}px)`;
        });
        el.addEventListener("mouseleave", () => {
            el.style.transform = "";
    ...
    function attachTilt(sel) {
        ...
        document.querySelectorAll(sel).forEach((el) => {
            el.addEventListener("mousemove", (e) => {
                const r = el.getBoundingClientRect();
                el.style.transform =
                    `perspective(900px) rotateX(${((e.clientY - r.top) / r.height - 0.5) * -7}deg) rotateY(${((e.clientX - r.left) / r.width - 0.5) * 7}deg)`;
                el.style.transition = "transform .1s";
            });
            el.addEventListener("mouseleave", () => {
                el.style.transform = "";
    ...
    function attachCardTilt() {
        ...
        document.querySelectorAll(".bcard").forEach((c) => {
            c.addEventListener("mousemove", (e) => {
                ...
                c.style.transform =
                    `perspective(700px) rotateY(${x * 7}deg) rotateX(${-y * 7}deg) translateZ(10px) translateY(-4px)`;
                c.style.transition = "transform .1s";
            });
            c.addEventListener("mouseleave", () => {
                c.style.transform = "";
                c.style.transition = "transform .6s cubic-bezier(.16,1,.3,1)";
            });
        });
    }
   ...
   Uses CSS grid-template-rows: 0fr ↔ 1fr
   Key requirements:
   1. .srv-grid has display:grid + transition on grid-template-rows
   2. Inner div MUST have overflow:hidden AND min-h-0
   3. JS simply toggles grid-template-rows value
════════════════════════════════════════════ */
    ...
    const PD = [{
        ...
        },
        {
            id: 10,
            t: "Boys High School",
            d: "Secondary school website with academic pages, events, staff directory, and online admission inquiries. First digital presence for the institution.",
    ...
    function renderP(f) {
        const list = f === "all" ? PD : PD.filter((p) => p.tags.includes(f));
        document.getElementById("pgrid").innerHTML = list
            .map((p, i) => {
                const span = SPANS[i] || 2,
                    tall = span >= 4;
                return `<div class="bcard col-span-6 md:col-span-${span} rounded-2xl border border-black/[0.07] dark:border-white/[0.07] bg-white dark:bg-card overflow-hidden cursor-pointer hover:border-black/14 dark:hover:border-white/13 hover:shadow-glow hover:-translate-y-1 transition-all duration-300"
      onclick="openModal(${p.id})" role="button" tabindex="0" onkeydown="if(event.key==='Enter')openModal(${p.id})">
      <div class="relative flex items-center justify-center overflow-hidden group" style="height:${tall ? "260px" : "190px"};background:linear-gradient(135deg,${p.g[0]}25,${p.g[1]}70)">
        <div class="text-[58px] select-none opacity-[.28] transition-transform duration-300 group-hover:scale-110">${p.e}</div>
        <div class="absolute inset-0 bg-ink/55 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          <div class="bg-white text-ink text-xs font-semibold px-5 py-2 rounded-lg translate-y-2 group-hover:translate-y-0 transition-transform duration-300">View Project ↗</div>
        </div>
      </div>
      <div class="p-5">
        <div class="flex flex-wrap gap-1.5 mb-2">${p.tags.map((t) => `<span class="tag">${t}</span>`).join("")}</div>
        <div class="font-display font-semibold text-[16px] text-[#0b1120] dark:text-white mb-1.5">${p.t}</div>
        <div class="text-[13px] text-[#8099b8] dark:text-white/40 leading-relaxed overflow-hidden" style="display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical">${p.d}</div>
        <div class="flex items-center gap-2 mt-3 pt-3 border-t border-black/[0.05] dark:border-white/[0.05] font-mono text-[11px] text-emerald-600 dark:text-emerald-400">
          <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="20 6 9 17 4 12"/></svg>${p.imp}
        </div>
      </div>
    </div>`;
            })
            .join("");
        attachCardTilt();
        if (window.gsap && !REDUCED) gsap.fromTo("#pgrid .bcard", {
            opacity: 0,
            y: 20
        }, {
            opacity: 1,
    ...
    window.filterP = function(f, btn) {
        document.querySelectorAll(".pfilt").forEach((b) => b.classList.remove("on"));
        if (btn) btn.classList.add("on");
        renderP(f);
    };

    // Init — activate "All"
    document.querySelector(".pfilt").classList.add("on");
    renderP("all");
    ...
    window.openModal = function(id) {
        ...
        const bg = document.getElementById("modbg"),
            box = document.getElementById("modbox"),
            th = document.getElementById("modthumb");
        th.style.background = `linear-gradient(135deg,${p.g[0]}55,${p.g[1]}99)`;
        th.innerHTML =
            `<button onclick="closeModal()" class="absolute top-3 right-3 h-8 w-8 grid place-items-center rounded-full bg-black/50 text-white/70 hover:text-white hover:rotate-90 transition-all duration-200 z-10">✕</button><div class="text-[90px] opacity-[.22] select-none">${p.e}</div>`;
        ...
        document.getElementById("modtags").innerHTML = p.tags
            ...
            .join("");
        document.getElementById("modimp").innerHTML = `<span class="font-bold">▶ Result: </span>${p.imp}`;
        document.getElementById("moddesc").textContent = p.d;
        document.getElementById("modacts").innerHTML =
            `
    <a href="${p.url}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-2 rounded-full bg-grad-main px-6 py-2.5 text-sm font-medium text-white shadow-glow hover:shadow-glow-v hover:-translate-y-0.5 transition-all duration-200" style="text-decoration:none">Visit Live Site ↗</a>
    <button onclick="closeModal()" class="inline-flex items-center gap-2 rounded-full border border-black/[0.09] dark:border-white/[0.1] bg-black/[0.02] dark:bg-white/[0.03] px-6 py-2.5 text-sm font-medium text-[#0b1120] dark:text-white/90 hover:bg-black/[0.04] dark:hover:bg-white/[0.07] transition-all duration-200">Close</button>`;
        bg.style.opacity = "1";
    ...
    document.getElementById("modbg").addEventListener("click", function(e) {
